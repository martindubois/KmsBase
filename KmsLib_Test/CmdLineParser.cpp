...
KMS_TEST_BEGIN(CmdLineParser_Base)
	...
	KMS_TEST_ASSERT(0 == lCLP0.GetArgumentCount	());
	KMS_TEST_ASSERT(0 == lCLP0.GetErrorFlags	());

	lCLP0.GetData(0, lStr, sizeof(lStr), "Default");
	KMS_TEST_ASSERT(0 == strcmp("Default", lStr));
	...
	KMS_TEST_ASSERT(0 == strcmp("10", lStr));

	lCLP1.GetData(1, &lUInt, 10);
	KMS_TEST_ASSERT(10 == lUInt);

	lCLP1.GetData(1, &lUInt, 16, 0x0);
	KMS_TEST_ASSERT(0x10 == lUInt);
