...
KMS_TEST_BEGIN(NetworkAddress_Base)
	...
	KMS_TEST_ASSERT(0 == WSAStartup(MAKEWORD(2, 2), &lData));
	...
	KMS_TEST_ASSERT(0 == lNA0.GetAddr());
	KMS_TEST_ASSERT(0 == lNA0.GetPort());

	KMS_TEST_ASSERT(sizeof(sockaddr_in) == lNA0.GetSize());

	lNA0.Set(0xffffffff);

	KMS_TEST_ASSERT(lNA0.IsBroadcast());

	KMS_TEST_ASSERT(0xffffffff == lNA0.GetAddr());
	...
	KMS_TEST_ASSERT(0xc2c1 == lNA0.GetPort());
